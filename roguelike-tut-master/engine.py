...
BACKGROUND_FILE = os.path.join(DATA_FOLDER, "human_probe_v1.png")
...
def main():
	...
	libtcodpy.console_init_root(constants["screen_width"], 
		constants["screen_height"], constants["window_title"], False)
		
	con = libtcodpy.console_new(constants["screen_width"], constants["screen_height"])
	panel = libtcodpy.console_new(constants["screen_width"], constants["screen_height"])
	...
	main_menu_background_image = libtcodpy.image_load(BACKGROUND_FILE)
	
	key = libtcodpy.Key()
	mouse = libtcodpy.Mouse()
	
	while not libtcodpy.console_is_window_closed():
		libtcodpy.sys_check_for_event(libtcodpy.EVENT_KEY_PRESS | libtcodpy.EVENT_MOUSE,
			key, mouse)
...
def play_game(player, entities, game_map, message_log, game_state, con, panel, constants):
	...
	while not libtcodpy.console_is_window_closed():
		...
		render_all(con, panel, entities, player, game_map, 
			fov_map, fov_recompute, message_log, constants["screen_width"], 
		...
		if pickup and game_state == GameStates.PLAYERS_TURN:
			for entity in entities:
				...
				message_log.add_message(Message("Tehre is nothing here to pick up.", libtcodpy.yellow))
